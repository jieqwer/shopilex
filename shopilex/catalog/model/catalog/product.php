	...
	public function getProducts($data = array()) {
		...
		if (!$product_data) {
			...
			if (isset($data['filter_tag']) && $data['filter_tag']) {
				$sql .= " AND p.product_id IN (SELECT pt.product_id FROM " . DB_PREFIX . "product_tag pt WHERE pt.language_id = '" . (int)$this->config->get('config_language_id') . "' AND LCASE(pt.tag) BINARY LIKE '%" . $this->db->escape(mb_strtolower($data['filter_tag'], 'UTF-8')) . "%')";
			}
